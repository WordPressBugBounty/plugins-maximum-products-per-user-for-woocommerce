...
if ( ! class_exists( 'WPFMPPU_My_Account' ) ) :

class WPFMPPU_My_Account {
	...
	function __construct() {
		if ( 'yes' === get_option( 'alg_wc_mppu_my_account_enabled', 'no' ) ) {
			...
			add_action( 'wpfmppu_on_activation', array( $this, 'plugin_activation' ) );
		}
	}
	...
	function init() {
		...
		add_action( 'wpfmppu_after_save_settings', array( $this, 'flush_rewrite_rules_on_save_frontend_settings' ) );
	...
	function icon() {
		if ( '' !== ( $icon_code = get_option( 'alg_wc_mppu_my_account_tab_icon', '' ) ) ) {
			?>
			<style>
                .woocommerce-MyAccount-navigation ul li.woocommerce-MyAccount-navigation-link--<?php echo esc_attr( $this->get_product_limits_tab_id() ); ?> a::before {
                    content: "\<?php echo esc_attr( $icon_code ); ?>";
                }
			</style>
			<?php
		}
	 ...
	 */
	function content() {
		echo do_shortcode( get_option( 'alg_wc_mppu_my_account_tab_content', '[wpfmppu_user_product_limits]' ) );
	}
	...
	function endpoint_title( $title ) {
		...
		if ( $is_endpoint && ! is_admin() && is_main_query() && in_the_loop() && is_account_page() ) {
			$title = esc_html( $this->get_tab_title() );
