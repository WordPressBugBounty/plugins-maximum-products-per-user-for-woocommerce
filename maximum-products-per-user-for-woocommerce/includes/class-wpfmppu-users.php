...
class WPFMPPU_Users {
	 ...
	 * @since 3.6.4
	 *
	 * @var WPFMPPU_Update_User_Sales_data_Bkg_Process
	 */
	 ...
	 * @since 3.8.6
	 *
	 * @var WPFMPPU_Update_User_Terms_data_Bkg_Process
	 */
	...
	function __construct() {
		...
		add_filter( 'wpfmppu_profile_page_table_row', array( $this, 'add_delete_user_sales_button' ), 10, 2 );
	...
	function show_user_sales_data_delete_notices() {
		if ( 'yes' !== wpfmppu_get_option( 'alg_wc_mppu_editable_sales_data', 'no' ) ) {
			return;
		}
		...
		if ( 'success' === $delete_result ) {
			echo '<div class="notice notice-success is-dismissible"><p>' .
			        esc_html__( 'User sales data has been successfully deleted.', 'maximum-products-per-user-for-woocommerce' ) .
			     '</p></div>';
		}
	...
	function add_delete_user_sales_button( $table_row, $user ) {
		if (
			'yes' !== wpfmppu_get_option( 'alg_wc_mppu_delete_user_sales_btn_enabled', 'no' ) ||
			'yes' !== wpfmppu_get_option( 'alg_wc_mppu_editable_sales_data', 'no' )
		) {
			return $table_row;
		}
		$button_url = add_query_arg(
			array(
				'wpfmppu_action' => 'delete_user_sales_data',
				'nonce'              => wp_create_nonce( 'wpfmppu_delete_user_sales_data' ),
				'user_id'            => $user->ID,
			),
			''
		);
	...
	function delete_user_sales_data_on_btn_click() {
		...
		if (
			! isset( $_GET['wpfmppu_action'] ) ||
			! isset( $_GET['nonce'] ) ||
			empty( $user_id ) ||
			'delete_user_sales_data' !== sanitize_text_field( wp_unslash( $_GET['wpfmppu_action'] ) ) ||
			'yes' !== wpfmppu_get_option( 'alg_wc_mppu_delete_user_sales_btn_enabled', 'no' ) ||
		...
		check_admin_referer( 'wpfmppu_delete_user_sales_data', 'nonce' );

		// User email type option.
		$user_email_type = wpfmppu_get_option( 'alg_wc_mppu_delete_user_sales_email_type', 'user_email' );
		...
		if ( $orders ) {
			foreach ( $orders as $order_id ) {
				wpfmppu()->core->data->delete_quantities( $order_id );
			}
		}
		...
		update_user_meta(
			get_current_user_id(),
			'alg_wc_mppu_delete_user_sales_notice',
			'success',
			30
		);
		wp_safe_redirect(
			add_query_arg(
				...
				remove_query_arg(
					array( 'wpfmppu_action', 'nonce', 'user_id' ),
					wp_get_referer()
				)
			)
		);
	...
	function init_bkg_process() {
		require_once( wpfmppu()->plugin_path() . '/includes/background-process/class-wpfmppu-update-user-sales-data-bkg-process.php' );
		$this->update_user_sales_bkg_process = new WPFMPPU_Update_User_Sales_data_Bkg_Process();

		require_once( wpfmppu()->plugin_path() . '/includes/background-process/class-wpfmppu-update-user-terms-data-bkg-process.php' );
		$this->update_user_terms_bkg_process = new WPFMPPU_Update_User_Terms_data_Bkg_Process();
	}
				...
				if ( is_array( $orders_data ) && isset( $orders_data[ $user_id ] ) ) {
					foreach ( $orders_data[ $user_id ] as $order_id => $order_data ) {
						$data = array(
							...
							'"' . $product_title . '"',
							$order_id,
							date_i18n( 'Y-m-d H:i:s', wpfmppu()->core->get_order_date( $order_data['date_created'] ) ),
							$order_data['qty'],
						);
	...
	function export_orders_data() {
		if (
			! isset( $_GET['wpfmppu_export_single_user_orders_data'] ) ||
			'yes' !== wpfmppu_get_option( 'alg_wc_mppu_editable_sales_data', 'no' )
		) {
			return;
		}
		check_admin_referer( 'wpfmppu_export_nonce' );

		$user_id = absint( wp_unslash( $_GET['wpfmppu_export_single_user_orders_data'] ) );
	...
	function export_orders_data_all_users() {
		if ( ! isset( $_GET['wpfmppu_export_all_users_orders_data'] ) ) {
			return;
		}
		check_admin_referer( 'wpfmppu_export_nonce' );
	...
	function update_profile_fields( $user_id ) {
		if (
			! isset( $_POST['wpfmppu_meta_nonce'] ) ||
			! wp_verify_nonce(
				sanitize_text_field( wp_unslash( $_POST['wpfmppu_meta_nonce'] ) ),
				'wpfmppu_save_meta'
			)
		) {
			return false;
		...
		// Totals data.
		if ( isset( $_POST['wpfmppu_totals_data'] ) ) {
			$post_totals_data = wp_unslash( $_POST['wpfmppu_totals_data'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			foreach ( $post_totals_data as $product_or_term => $data ) {
				foreach ( $data as $product_or_term_id => $qty ) {
					$this->update_totals_data(
						array(
							'user_id'            => $user_id,
							'product_or_term'    => sanitize_key( $product_or_term ),
							'product_or_term_id' => absint( $product_or_term_id ),
							'qty'                => sanitize_text_field( $qty ),
						)
					);
				}
			}
		...
		// Orders data.
		if ( isset( $_POST['wpfmppu_orders_data'] ) ) {
			$iteration_count = 0;
			$post_orders_data = wp_unslash( $_POST['wpfmppu_orders_data'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			foreach ( $post_orders_data as $product_or_term => $data ) {
			...
			if ( $iteration_count < get_option( 'alg_wc_mppu_bkg_process_min_amount', 50 ) ) {
				...
				if ( $this->update_user_sales_bkg_process->is_active() ) {
					$this->update_user_sales_bkg_process->cancel();
				}
				foreach ( $post_orders_data as $product_or_term => $data ) {
					foreach ( $data as $product_or_term_id => $order_data ) {
						$this->update_user_sales_bkg_process->push_to_queue(
							array(
								'user_id'            => $user_id,
								'product_or_term'    => sanitize_key( $product_or_term ),
								'product_or_term_id' => absint( $product_or_term_id ),
				...
				update_user_meta(
					get_current_user_id(),
					'alg_wc_mppu_profile_updated',
					array( 'method' => 'bkg_process' )
				);
			}
	...
	function calculate_terms_data_from_products_data( $user_id ) {
		...
		if (
			...
			$product = (
				isset( $_POST['wpfmppu_orders_data']['product'] ) &&
				is_array( $_POST['wpfmppu_orders_data']['product'] )
			) ?
				map_deep( wp_unslash( $_POST['wpfmppu_orders_data']['product'] ), 'sanitize_text_field' ) :
				array();
	...
	function update_terms_data( $args = null ) {
		...
		$totals_data     = wpfmppu()->core->get_post_or_term_meta( 'term', $term->term_id, '_alg_wc_mppu_totals_data' );
		$totals_data_new = $totals_data;
		$orders_data     = wpfmppu()->core->get_post_or_term_meta( 'term', $term->term_id, '_alg_wc_mppu_orders_data' );
	...
	function get_terms_query( $taxonomy ) {
		if (
			'yes' === apply_filters( 'wpfmppu_' . $taxonomy . '_enabled', 'no' ) &&
	...
	function update_orders_data( $args = null ){
		...
		$lifetime_totals    = 0;
		$orders_data        = wpfmppu()->core->get_post_or_term_meta( $product_or_term, $product_or_term_id, '_alg_wc_mppu_orders_data' );
		foreach ( $order_data as $order_id => $qty ) {
			$orders_data[ $user_id ][ $order_id ]['qty'] = intval( $qty );
			$lifetime_totals                             += intval( $qty );
		}
		wpfmppu()->core->update_post_or_term_meta( $product_or_term, $product_or_term_id, '_alg_wc_mppu_orders_data', $orders_data );
		// Update totals data.
	...
	function update_totals_data( $args = null ) {
		...
		$user_id                 = $args['user_id'];
		$qty                     = $args['qty'];
		$totals_data             = wpfmppu()->core->get_post_or_term_meta( $product_or_term, $product_or_term_id, '_alg_wc_mppu_totals_data' );
		$totals_data[ $user_id ] = intval( $qty );
		wpfmppu()->core->update_post_or_term_meta( $product_or_term, $product_or_term_id, '_alg_wc_mppu_totals_data', $totals_data );
	}
	...
	function get_user_products_data_html( $user ) {
		...
		if ( ! empty( $output ) ) {
			echo '<table class="widefat striped alg_wc_mppu_products_data">' .
					'<tr>' .
						'<th>' . esc_html__( 'Product', 'maximum-products-per-user-for-woocommerce' ) . '</th>' .
						'<th>' . esc_html__( 'Lifetime', 'maximum-products-per-user-for-woocommerce' ) . '</th>' .
						'<th>' . esc_html__( 'Orders', 'maximum-products-per-user-for-woocommerce' ) . '</th>' .
					'</tr>' .
					wp_kses( $output, wpfmppu()->core->get_allowed_form_html() ) .
				'</table>';
		}
	...
	function get_user_terms_data_html( $user, $taxonomy ) {
		...
		if ( 'yes' === apply_filters( 'wpfmppu_' . $taxonomy . '_enabled', 'no' ) ) {
			...
			if ( ! empty( $output ) ) {
				echo '<table class="widefat striped alg_wc_mppu_' . esc_attr( $taxonomy ) . '_data">' .
						'<tr>' .
	...
	function show_extra_profile_fields( $user ) {
		...
		$sales_data_btn_html = $sales_data_btn->get_btn_html(
			array(
				'user_id' => $user->ID,
		...
		$url = wp_nonce_url(
				add_query_arg( 'wpfmppu_export_single_user_orders_data', intval( $user->ID ) ),
				'wpfmppu_export_nonce'
			);

		$export_sales_data_html = sprintf(
			'<a class="mppu-export-data button button-primary" href="%s"><i class="dashicons dashicons-download"></i>%s</a>',
			esc_url( $url ),
			esc_html__( 'Export products data', 'maximum-products-per-user-for-woocommerce' )
		);

		echo wp_kses( $style, wpfmppu()->core->get_allowed_style() );
		echo '<h2>' . esc_html__( 'Maximum Products per User', 'maximum-products-per-user-for-woocommerce' ) . '</h2>';
		echo '<table class="form-table" id="mppu-products-data">' .
				'<tr>'.
					'<th>' .
						esc_html__( 'Export Data', 'maximum-products-per-user-for-woocommerce' ) .
					'</th>' .
					'<td>' . wp_kses_post( $export_sales_data_html ) . '</td>' .
				'</tr>' .
		        wp_kses_post( apply_filters( 'wpfmppu_profile_page_table_row', '', $user ) ) .
				'<tr>' .
					'<th>' .
						esc_html__( 'Products Data', 'maximum-products-per-user-for-woocommerce' ) .
					'</th>' .
					'<td>' . wp_kses( $product_data_html, wpfmppu()->core->get_allowed_form_html() ) . '</td>' .
				'</tr>' .
				wp_kses( $terms_data, wpfmppu()->core->get_allowed_form_html() ) .
			'</table>';
		wp_nonce_field(
			'wpfmppu_save_meta',
			'wpfmppu_meta_nonce'
	...
	function get_user_profile_styles() {
		...
		$sales_data_btn_style = new WPFMPPU_Sales_Data_Btn();
	...
	function get_extra_profile_fields_terms_data_table_rows( $user ) {
		...
		foreach ( array( 'product_cat', 'product_tag' ) as $taxonomy ) {
			if ( 'yes' === apply_filters( 'wpfmppu_' . $taxonomy . '_enabled', 'no' ) ) {
				$term_name  = 'product_cat' === $taxonomy ?
	...
	function handle_sales_data_via_js() {
		...
		$php_to_js = array(
			'security' => wp_create_nonce( 'wpfmppu_get_user_sales_data' ),
		);
		?>
        <script>
	...
	function get_user_sales_data_html_ajax() {
		check_ajax_referer( 'wpfmppu_get_user_sales_data', 'security' );
		if (
			'yes' !== wpfmppu_get_option( 'alg_wc_mppu_editable_sales_data', 'no' )
		) {
			return;
		...
		$data_type = sanitize_key( $args['data_type'] );

		if ( ! in_array( $data_type, array( 'product', 'product_tag', 'product_cat' ), true ) ) {
			$data_type = 'product';
		}
	 ...
	 * @version 4.5.0
	 * @since   2.2.0
	 * @todo    [next] use `wpfmppu()->core->get_date_format()`
	 */
	function get_item_data( $user, $product_or_term_id, $product_or_term, $product_or_term_title ) {
		...
		$do_add_empty_totals = ( 'yes' === wpfmppu_get_option( 'alg_wc_mppu_editable_sales_data_empty_totals', 'no' ) );
		if ( $do_add_empty_totals || isset( $totals_data[ $user->ID ] ) || isset( $orders_data[ $user->ID ] ) ) {
			...
			if ( $do_add_empty_totals || isset( $totals_data[ $user->ID ] ) ) {
				$name  = 'wpfmppu_totals_data[' . $product_or_term . '][' . $product_or_term_id . ']';
			...
			if ( isset( $orders_data[ $user->ID ] ) ) {
				...
				foreach ( $orders_data[ $user->ID ] as $order_id => $order_data ) {
					$name  = 'wpfmppu_orders_data[' . $product_or_term . '][' . $product_or_term_id . '][' . $order_id . ']';
					...
					$orders[ $order_id ] = '<input type="number" name="' . $name . '" value="' . $value . '"'.$orders_input_disabled_str.'>' .
						...
						wc_help_tip(
							sprintf(
								/* Translators: %1$d: Order ID, %2$s: Order date. */
								__( 'Order #%1$d<br>(%2$s)', 'maximum-products-per-user-for-woocommerce' ),
							$order_id, date_i18n( 'Y-m-d H:i:s', wpfmppu()->core->get_order_date( $order_data['date_created'] ) )
							),
							true
						);
				}
