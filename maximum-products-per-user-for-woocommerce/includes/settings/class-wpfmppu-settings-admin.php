...
if ( ! class_exists( 'WPFMPPU_Settings_Admin' ) ) :

class WPFMPPU_Settings_Admin extends WPFMPPU_Settings_Section {
	...
	function get_settings() {
		$product_sales_data_opts = array(
			array(
				...
				'desc'  => sprintf(
					/* Translators: %s: Admin Product page text. */
					__( 'See sales data from products on the %s.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'admin product page', 'maximum-products-per-user-for-woocommerce' )
				),
				'type'  => 'title',
		...
		$users_editable_sales_data_opts = array(
			array(
				...
				'desc'     => sprintf(
					/* Translators: %s: Link. */
					__( 'Manage sales data from users on the %s.', 'maximum-products-per-user-for-woocommerce' ),
					'<a href="' . admin_url( 'profile.php' ) . '">' . __( 'profile page', 'maximum-products-per-user-for-woocommerce' ) . '</a>'
				),
				'type'     => 'title',
			...
			array(
				...
				'desc'     => sprintf(
					...
					__( 'Show variations even if %s option is disabled', 'maximum-products-per-user-for-woocommerce' ),
					'<strong>' . __( 'Use Variations', 'maximum-products-per-user-for-woocommerce' ) . '</strong>'
				),
		...
		$user_sales_data_export_opts = array(
			array(
				...
				'desc' => sprintf(
					...
					esc_url(
						wp_nonce_url(
							add_query_arg( 'wpfmppu_export_all_users_orders_data', true ),
							'wpfmppu_export_nonce'
						)
					),
					esc_html__( 'Export sales data for all users', 'maximum-products-per-user-for-woocommerce' )
				),
				'type'     => 'title',
			...
			array(
				...
				'desc_tip'  => sprintf(
					/* Translators: %s: Export button text. */
					__( 'Used in "%s" tool.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'Export sales data for all users', 'maximum-products-per-user-for-woocommerce' )
				),
			...
			array(
				...
				'desc'     => sprintf(
					/* Translators: %s: Option name. */
					__( 'Ignored unless "%s" option is enabled.', 'maximum-products-per-user-for-woocommerce' ),
					__( 'Merge user', 'maximum-products-per-user-for-woocommerce' )
				),
...
return new WPFMPPU_Settings_Admin();
