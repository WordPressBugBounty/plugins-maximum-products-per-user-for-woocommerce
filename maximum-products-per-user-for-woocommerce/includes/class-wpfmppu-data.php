...
if ( ! class_exists( 'WPFMPPU_Data' ) ) :

class WPFMPPU_Data extends WPFMPPU_Dynamic_Properties_Obj {
	 ...
	 * @since 3.6.4
	 *
	 * @var WPFMPPU_Recalculate_Sales_Bkg_Process
	 */
	 ...
	 * $since 3.6.4.
	 *
	 * @var WPFMPPU_Delete_Sales_Async_Request;
	 */
	...
	function __construct() {
		...
		// Calculate data.
		add_action( 'wpfmppu_after_save_settings', array( $this, 'calculate_data' ) );
	...
	function deduct_refunds_from_orders( $order_id, $refund_id ) {
		if ( 'yes' === wpfmppu_get_option( 'alg_wc_mppu_deduct_refunds', 'no' ) ) {
	...
	function init_bkg_process() {
		require_once( wpfmppu()->plugin_path() . '/includes/background-process/class-wpfmppu-recalculate-sales-bkg-process.php' );
		$this->recalculate_sales_bkg_process = new WPFMPPU_Recalculate_Sales_Bkg_Process();
	...
	function handle_delete_sales_async_request_init() {
		if (
			...
			(
				...
				(
					isset( $_POST['alg_wc_mppu_tool_delete_using_async_request'] ) && // phpcs:ignore WordPress.Security.NonceVerification.Missing
					true === wp_validate_boolean( sanitize_text_field( wp_unslash( $_POST['alg_wc_mppu_tool_delete_using_async_request'] ) ) ) // phpcs:ignore WordPress.Security.NonceVerification.Missing
				)
			)
		) {
			require_once( wpfmppu()->plugin_path() . '/includes/background-process/class-wpfmppu-delete-sales-async-request.php' );
			$this->delete_sales_async_request = new WPFMPPU_Delete_Sales_Async_Request();
		}
	}
	...
	function calculate_data_notice() {
		...
		if ( $this->admin_notice_data['order_num'] > 0 ) {
			if ( ! $this->recalculate_sales_bkg_process_used ) {
				$message .= sprintf(
					...
					_n( '%s order processed.', '%s orders processed.', $this->admin_notice_data['order_num'], 'maximum-products-per-user-for-woocommerce' ),
				...
				$message .= 'yes' === get_option( 'alg_wc_mppu_bkg_process_send_email', 'yes' ) ?
					' ' . sprintf(
						/* Translators: %s: Email address. */
						__( 'When it is complete an e-mail is going to be sent to %s.', 'maximum-products-per-user-for-woocommerce' ),
						get_option( 'alg_wc_mppu_bkg_process_email_to', get_option( 'admin_email' ) )
					) :
					'';
				$message .= '<br>' . sprintf(
					/* Translators: %s: Number of orders. */
					_n( '%s order being processed.', '%s orders being processed.',
						$this->admin_notice_data['order_num'], 'maximum-products-per-user-for-woocommerce' ),
						'<strong>' . $this->admin_notice_data['order_num'] . '</strong>'
					);
			}
		}
		if ( $this->admin_notice_data['meta_num'] > 0 ) {
			...
			$message .= sprintf(
				/* Translators: %s: Number of metadata items. */
				_n( '%s meta deleted.', '%s metas deleted.', $this->admin_notice_data['order_num'], 'maximum-products-per-user-for-woocommerce' ),
		...
		if ( $this->admin_notice_data['order_num'] > 0 && 'yes' === get_option( 'alg_wc_mppu_tool_recalculate_debug', 'no' ) ) {
			$debug_message = '[' . __( 'Debug', 'maximum-products-per-user-for-woocommerce' ) . ']' . ' ' .
			                 sprintf(
								 ...
								 __( 'Orders: %s.', 'maximum-products-per-user-for-woocommerce' ),
			...
			$debug_message .= '<br />' . sprintf(
				/* Translators: %s: Seconds. */
				__( 'Task took %s second(s).', 'maximum-products-per-user-for-woocommerce' ),
				number_format( $seconds_spent, 2 )
				);
			printf(
				'<div class="%1$s"><p>%2$s</p></div>',
				esc_attr( $class ),
				wp_kses_post( $debug_message )
			);
		}
	}
	...
	function delete_meta_data() {
		...
		$query_results        = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
				"SELECT * FROM $wpdb->postmeta WHERE meta_key IN {$in_str}", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$keys
			)
		);
		foreach ( $query_results as $meta ) {
			delete_post_meta( $meta->post_id, $meta->meta_key );
			$counter ++;
		}
		if ( \Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled() ) {
			$in_str        = $this->generate_wpdb_prepare_placeholders_from_array( $order_keys );
			$query_results = $wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
				$wpdb->prepare(
					"DELETE FROM {$wpdb->prefix}wc_orders_meta WHERE meta_key IN {$in_str}", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
					$order_keys
				)
			);
		...
		$query_results  = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
			$wpdb->prepare(
				"SELECT * FROM $wpdb->termmeta WHERE meta_key IN {$in_str}", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$keys
			)
		);
		foreach ( $query_results as $meta ) {
		...
		if ( $do_recalculate || $do_delete_recalculate || $do_delete ) {
			...
			if ( $do_recalculate || $do_delete_recalculate ) {
				...
				if ( $do_add_date_query ) {
					$date_query = array(
						'after' => gmdate( 'Y-m-d H:i:s', wpfmppu()->core->get_date_to_check( array(
				...
				while ( true ) {
					...
					if ( $do_wp_query ) {
						...
						if (
							...
							if ( $this->recalculate_sales_bkg_process->is_active() ) {
						...
						// Loop
						$orders = wc_get_orders( apply_filters( 'wpfmppu_calculate_data_wc_get_orders_args', $args ) );
			...
			add_action( 'woocommerce_sections_' . 'wpfmppu', array( $this, 'calculate_data_notice' ) );
		}
	}
	...
	function transaction_update_meta( $product_or_term_id, $meta_key, $meta_value, $is_product ) {
		...
		if( $update_meta_func( $product_or_term_id, $meta_key, $meta_value ) ) {
	...
	function get_user_id_from_order( $order ) {
		$user_id = ( wpfmppu()->core->is_wc_version_below_3_0_0 ? $order->customer_user : $order->get_customer_id() );
		if ( ! $user_id && wpfmppu()->core->do_identify_guests_by_ip ) {
			$user_id = 'ip:' . $order->get_customer_ip_address();
		}

		if ( ! $user_id && wpfmppu()->core->do_identify_by_checkout_email ) {
	...
	function update_quantities( $args = null ) {
		...
		if ( $order = wc_get_order( $order_id ) ) {
			...
			if ( ( $do_save && ! $is_data_saved ) || ( ! $do_save && $is_data_saved ) ) {
				if ( ! apply_filters( "wpfmppu_{$action}_quantities", true, $order_id, $order ) ) {
					return;
				}
				...
				if ( sizeof( $order->get_items() ) > 0 ) {
					...
					foreach ( $order->get_items() as $item ) {
						if ( $item->is_type( 'line_item' ) && ( $product = $item->get_product() ) ) {
							$parent_product_id = wpfmppu()->core->get_parent_product_id( $product );
							$product_id        = wpfmppu()->core->get_product_id( $product );
							$product_qty       = apply_filters( 'wpfmppu_save_quantities_item_qty', $item->get_quantity(), $item, $order );
							...
							if ( ! empty( $exclude_products ) && in_array( ( wpfmppu()->core->do_use_variations( $parent_product_id ) ? $product_id : $parent_product_id ), $exclude_products ) ) {
								continue;
							}
							...
							foreach ( $products_and_terms as $object_data ) {
								...
								if ( $do_save ) {
									...
									} elseif ( apply_filters( 'wpfmppu_orders_data_increase_qty', true, $order_id, $user_id, $product_or_term_id, $is_product ) ) {
								...
								if ( $do_save ) {
									...
									if (
										...
										apply_filters( 'wpfmppu_totals_data_increase_qty', true, $user_id, $product_or_term_id, $is_product )
									) {
									...
									if (
										isset( $users_quantities[ $user_id ] ) &&
										apply_filters( 'wpfmppu_totals_data_decrease_qty', true, $user_id, $product_or_term_id, $is_product )
									) {
								...
								$this->transaction_update_meta( $product_or_term_id, '_alg_wc_mppu_totals_data', apply_filters( 'wpfmppu_totals_data', $users_quantities, $user_id, $product_or_term_id, $is_product, $users_orders_quantities ), $is_product );
							}
						}
					}
				}
				do_action( 'wpfmppu_order_data_saved', array(
					'order'   => $order,
